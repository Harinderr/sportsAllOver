   ...
   useEffect(()=> {
    ...
    const handleSearch = async () => {
        try {
            ...
            const response = await fetch(`/api/search?postslug=${postslug}`)
            if(response.ok) {
    ...
    return (
        <>
         ...
         {data.map((val)=> {
            return <Post key={val.id} src={val.img} slug={val.slug} title={val.title} content={val.subDes} date ={val.createdAt} id = {val.id}></Post> 
        })}
        </div>
         </div>
