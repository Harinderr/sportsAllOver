...
export default function Comment({ slug}) {
  ...
  const  commentData = async () => {
    try {
        let response = await fetch(` https://next-blog-sand-ten-63.vercel.ap/api/comment?slug=${slug}`)
        if(response.ok) {
...
async   function handleLogin(e) {
    ...
    try {
        const response = await fetch(` https://next-blog-sand-ten-63.vercel.ap/api/comment?slug=${slug}`, {
            method : 'POST',
            headers: {
